...
def plot_true_simulation_verify(alpha_c_half_true, ifp_half_true, alpha_c_full_true, ifp_full_true, 
    ...
    """
    Translation of Plot_True_simulation_Verify.m
    Visualizes the 'True' (synthetic) data for verification.
    """
    # Extraction and reshaping of the grid dimensions
    Nx = X.shape[1]
    Ny = X.shape[0]
    dim = (Ny, Nx)
    pdim = Nx * Ny
    ...
    # Extraction and reshaping of the state vector components from the initial ensemble
    # Order : alpF, khat_w, T_vess, T_lymp, Kgrow, Thetagrow
    ...
    # Reshaping of the 'True' simulation results
    ...
    # Display the mid-term data (Half)
    draw_plots(alpC_half, ifp_half, "Half")
    plt.pause(2)

    # Update with the final data (Full)
