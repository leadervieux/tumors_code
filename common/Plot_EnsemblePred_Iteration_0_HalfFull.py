...
def plot_half_full(alpha_c_with_unc, initial_ensemble, measured_values_half, measured_values_full, ifp_half, ifp_full, X, Y):
    """
    Translation of Plot_EnsemblePred_Iteration_0_HalfFull.m
    Displays the initial state and the predictions (Half/Full) of the tumor.
    """
    Nx = X.shape[1]
    Ny = X.shape[0]
    dim = (Ny, Nx)
    ...
    # Extraction and reshaping of the state vector components
    ...
    # Reshaping of the simulation results
    ...
    plt.ion() # Interactive mode to allow display during the loop
    ...
    # Display the mid-term prediction (Half)
    draw_all(alpC_half, ifp_half_val, "Half")
    print("Affichage Prediction Half...")
    plt.pause(0.1)

    # Update with the final prediction (Full)
