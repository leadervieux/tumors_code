...
def three_phase_simulator_compartment_full(alpha_c, initialEnsemble, Inc_dt, member_idx=0):
    #####################
    # 1. Discretization #
    #####################
    ...
    # Définition des zones d'indexation (Équivalents Python de Jx, J1x, etc.)
    # On utilise des slices pour la rapidité NumPy
    Jx = slice(0, Nx)        # 1:Nx
    J1x =  slice(1, Nx-1)     # 2:Nx-1 (cellules intérieures)
    ...
    t = 0.0  # Initialisation du temps pour la sauvegarde initiale
    dt = T_val / NTime
    dx = Lx / Nx
    i_dx = 1.0 / dx
    dy = Ly / Ny
    ...
    # Initialisation des variables de demi-mailles (L, R, B, T) non définies en section 3
    lam_c_half_L = np.zeros(dim); lam_c_half_R = np.zeros(dim)
    lam_c_half_B = np.zeros(dim); lam_c_half_T = np.zeros(dim)
    lam_f_half_L = np.zeros(dim); lam_f_half_R = np.zeros(dim)
    lam_f_half_B = np.zeros(dim); lam_f_half_T = np.zeros(dim)
    ...
    # Correction: Upwinding pour frac_flow_F_half_mod_y comme en MATLAB
    ...
    ###########################
    # Construction des matrices tridiagonales pour la diffusion implicite
    # Ces matrices sont de taille (N-2) car on ne résout que pour l'intérieur.
    ...
    # --- Pré-factorisation pour accélérer la boucle (Nouveau) ---
    ...
    # --- PRÉ-ALLOCATION (Crucial pour la vitesse et éviter les NameError) ---
    ...
    # Pour la validation, utilisez 1. Pour le run de 100 membres, utilisez 5.
    pressure_freq = 1
    ...
    for jj in range(NTime):
        # Variable de contrôle pour ne sauvegarder que l'itération 0 du premier membre
        # pour éviter de ralentir la simulation de l'ensemble
        is_debug = (jj == 0 and member_idx == 0 and Inc_dt == 0)

        t = (jj + 1) * dt # Correspondance exacte avec MATLAB

        # 7.1 Step 0: dt/2 reaction (Synchronisation avec MATLAB Source_cell)
        Sc = source_cell_theta(alpC_old, ecm_old, lam_11, Thetagrow)
        Sc = Sc * Kgrow
        Sf = source_fibroblast_new(alpF_old, ecm_old, lam_61, lam_62, lam_63)

        # Utilisation de l'exponentielle pour coller à l'intégration MATLAB
        ...
        # --- OPTIMISATION MASSIVE : Calcul groupé des coefficients ---
        # On calcule les puissances une seule fois (opération la plus lourde)
        ac_rc = alpC**rc
        af_rf = alpF**rf
        ...
        ac_2_rcf = alpC**(2 - rcf)
        af_2_rfc = alpF**(2 - rfc)
        aw_val = 1.00001 - alpC - alpF # Pas de clip : permet à aw^2 d'être élevé si sum > 1
        aw_2_rw = aw_val**(2 - rw)
        ...
        # Dénominateur partagé pour les mobilités (lam_c, lam_f, lam_w)
        ...
        # --- OPTIMISATION : Résolution de la pression moins fréquente ---
        ...
        ##########################
        # On réutilise den_flow et les termes déjà calculés (Similaire à MATLAB)
        ...
        alpC = np.maximum(alpC, 0.0000001)
        alpF = np.maximum(alpF, 0.0000001)
        alpC = np.minimum(alpC, 0.7) # Rétablissement de la limite stricte MATLAB
        ...
        if jj == int(NTime / 2) - 1:
            pred_alpha_c_half = alpC.flatten(order='F')
            pressureHalf = pW.flatten(order='F')
            
            if member_idx == 0 and Inc_dt == 0:
                half_data = {
                    'alpC_half': np.array(alpC, order='F'),
                    'alpF_half': np.array(alpF, order='F'),
                    'pW_half': np.array(pW, order='F'),
                    'a_half': np.array(a, order='F'),
                    'b_half': np.array(b, order='F'),
                    'c_half': np.array(c, order='F'),
                    'ecm_half': np.array(ecm, order='F'),
                    'jj_half': jj
                }
                savemat('debug_half_time_python.mat', half_data)
                print(f">>> Sauvegarde MI-PARCOURS effectuée à jj={jj}")


    # --- SAUVEGARDE DE DIAGNOSTIC (Fin du simulateur) ---
    if member_idx == 0 and Inc_dt == 0:
        diag_end = {
            'alpC_end': np.array(alpC, order='F'), # Correction: alpC_old n'est pas défini ici, utiliser alpC
            'alpF_end': np.array(alpF, order='F'),
            'pW_end': np.array(pW, order='F'),
            'a_end': np.array(a, order='F'),
            'b_end': np.array(b, order='F'),
            'c_end': np.array(c, order='F'),
            'ecm_end': np.array(ecm, order='F')
        }
        savemat('debug_final_state_python.mat', diag_end)
        print(">>> Diagnostic final sauvegardé dans debug_final_state_python.mat")

    pred_alpha_c_full = alpC.flatten(order='F')
    pressureEnd = pW.flatten(order='F')
    return pred_alpha_c_half, pred_alpha_c_full, pressureHalf, pressureEnd

        # --- Les solveurs ADI pour Protease, Chemokine et TGF suivront dans le prochain bloc ---
